...
namespace pokergame::core {
    ...
    //     };
    //
    //     // Pushes one action onto getPlayerAction's detail queue.
    ...
    //     // The current_bet_to_match becomes 5 (max of SB 5 and BB 5).
    //     // P1 already has 5 in, so they owe 0. They check.
    //     {
    //         resetSeats({{SeatState::Empty, 0}, {SeatState::InHand, 100}, {SeatState::InHand, 5}});
    //         this->dealer = 0; // Dealer is seat 0 (Empty)
    //         this->state = GameState::PreFlop;
    //         setActions({{BetType::Check, std::nullopt}});
    //         auto bets = handleBettingRound();
    //         assert(bets[1] == 5); // SB posted 5, then checked
    //         assert(bets[2] == 5); // BB posted 5 (all-in)
    ...
    //     // P0(D), P1(SB)=100, P2(BB)=5.
    //     // Pre-flop: P1 posts 5, P2 posts 5 (All-In).
    //     // Action starts at P0 (UTG in 3-handed). P0 raises to 20.
    //     // P1 must call 15 more (total 20). P2 is All-In, so no action.
    //     {
    //         resetSeats({{SeatState::InHand, 500}, {SeatState::InHand, 100}, {SeatState::InHand, 5}});
    //         this->dealer = 0;
    //         this->state = GameState::PreFlop;
    //         // P1 posts SB 5, P2 posts BB 5 (All-In).
    //         // first-to-act is P0.
    //         setActions({
    //             {BetType::Raise, 20}, // P0 raises to 20
    //             {BetType::Call, 15}   // P1 calls 15 more
    //         });
    //         auto bets = handleBettingRound();
    //         assert(bets[0] == 20);
    //         assert(bets[1] == 20);
    //         assert(bets[2] == 5);
    //         assert(seats[0].chips == 480);
    //         assert(seats[1].chips == 80);
    //         assert(seats[2].chips == 0);
    //         assert(seats[2].seat_state == SeatState::AllIn);
    //     }
    //
    //     // HB17: Heads-Up (One-on-One) Betting Action.
    //     // P0(D), P1(Non-Dealer).
    //     // In this implementation's logic:
    //     // Pre-flop: nextBetter(D) -> P1 posts SB (5), nextBetter(P1) -> P0 posts BB (10).
    //     // First to act: nextBetter(BB=0) -> P1.
    //     {
    //         resetSeats({{SeatState::InHand, 500}, {SeatState::InHand, 500}});
    //         this->dealer = 0;
    //         this->state = GameState::PreFlop;
    //         // P1 posts SB 5, P0 posts BB 10.
    //         // P1 calls 5 more. P0 checks.
    //         setActions({
    //             {BetType::Call, 5},
    //             {BetType::Check, std::nullopt}
    //         });
    ...
    //         assert(bets[0] == 10);
    //         assert(bets[1] == 10);
    //         assert(seats[0].chips == 490);
    //         assert(seats[1].chips == 490);
    //     }
    //
    ...
    //         processBetsOnRoundConclusion(bets);
    //         assert(this->pots.size() == 1);
    //         assert(this->pots[0].amount == 30);
    //         assert(this->pots[0].participants.size() == 3);
    //         assert(this->max_pot_sizes.empty());
    //     }
    //
    //     // PBORC2: One player all-in for less.
    //     {
    //         resetSeats({{SeatState::AllIn, 0}, {SeatState::InHand, 100}, {SeatState::InHand, 100}});
    //         // P0 all-in for 5, P1 and P2 bet 10.
    //         std::unordered_map<size_t, bet_t> bets = {{0, 5}, {1, 10}, {2, 10}};
    //         processBetsOnRoundConclusion(bets);
    //         // Expected:
    //         // Pot 0: 5*3 = 15. Participants: {0, 1, 2}. Max size: 5.
    //         // Pot 1: 5*2 = 10. Participants: {1, 2}.
    //         assert(this->pots.size() == 2);
    //         assert(this->pots[0].amount == 15);
    //         assert(this->pots[0].participants.size() == 3);
    //         assert(this->pots[1].amount == 10);
    //         assert(this->pots[1].participants.size() == 2);
    //         assert(this->max_pot_sizes.size() == 1);
    //         assert(this->max_pot_sizes[0] == 5);
    //     }
    //
    //     // PBORC3: Two players all-in for different amounts.
    //     {
    //         resetSeats({{SeatState::AllIn, 0}, {SeatState::AllIn, 0}, {SeatState::InHand, 100}});
    //         // P0: 5 (all-in), P1: 15 (all-in), P2: 20.
    //         std::unordered_map<size_t, bet_t> bets = {{0, 5}, {1, 15}, {2, 20}};
    //         processBetsOnRoundConclusion(bets);
    //         // Expected:
    //         // Pot 0: 5*3 = 15. Participants: {0, 1, 2}. Max: 5.
    //         // Pot 1: (15-5)*2 = 20. Participants: {1, 2}. Max: 10.
    //         // Pot 2: (20-15)*1 = 5. Participants: {2}.
    //         assert(this->pots.size() == 3);
    //         assert(this->pots[0].amount == 15);
    //         assert(this->pots[1].amount == 20);
    //         assert(this->pots[2].amount == 5);
    //         assert(this->max_pot_sizes.size() == 2);
    //         assert(this->max_pot_sizes[0] == 5);
    //         assert(this->max_pot_sizes[1] == 10);
    //     }
    //
    //     // PBORC4: Folded player contributes but doesn't participate.
    //     {
    //         resetSeats({{SeatState::InHand, 100}, {SeatState::Folded, 0}});
    //         // P1 folds after betting 5. P0 bets 10.
    //         std::unordered_map<size_t, bet_t> bets = {{1, 5}, {0, 10}};
    //         processBetsOnRoundConclusion(bets);
    //         // Expected:
    //         // Pot 0: 5+10 = 15. Participants: {0}. (P1 is folded)
    //         assert(this->pots.size() == 1);
    //         assert(this->pots[0].amount == 15);
    //         assert(this->pots[0].participants.size() == 1);
    //         assert(this->pots[0].participants.contains(0));
    //     }
    //
    //     // PBORC5: Multi-round side pot contribution.
    //     {
    //         resetSeats({{SeatState::AllIn, 0}, {SeatState::InHand, 100}, {SeatState::InHand, 100}});
    //         // Round 1: P0: 10 (all-in), P1: 10, P2: 10.
    //         std::unordered_map<size_t, bet_t> bets1 = {{0, 10}, {1, 10}, {2, 10}};
    //         processBetsOnRoundConclusion(bets1);
    //         assert(this->pots.size() == 1);
    //         assert(this->pots[0].amount == 30);
    //         assert(this->max_pot_sizes.size() == 1);
    //         assert(this->max_pot_sizes[0] == 10);
    //
    //         // Round 2: P1: 20, P2: 20.
    //         std::unordered_map<size_t, bet_t> bets2 = {{1, 20}, {2, 20}};
    //         processBetsOnRoundConclusion(bets2);
    //         // P1 and P2 each contribute 20.
    //         // Pot 0 is capped at 10 per player round
    //         assert(this->pots.size() == 2);
    //         assert(this->pots[0].amount == 50); // 30 (R1) + 10 (P1 R2) + 10 (P2 R2)
    //         assert(this->pots[1].amount == 20); // 10 (P1 R2) + 10 (P2 R2)
    //     }
    //
    ...
    //         processBetsOnRoundConclusion(bets);
    //         assert(this->pots.size() == 1);
    //         assert(this->pots[0].amount == 100);
    //         assert(this->max_pot_sizes.size() == 1);
    //         assert(this->max_pot_sizes[0] == 50);
    //     }
    //
